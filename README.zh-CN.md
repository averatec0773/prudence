...
<p align="center">
  面向用 AI 编码代理写软件的开发者的成长教练。<br>
  它把你和代理的工作方式，和代码在 git 里的后续命运联系起来。
</p>
...
用 AI 代理写代码的开发者交付得比以往都快，但很多人说不清自己有没有变强。代码能跑，却没人告诉你哪些工作习惯产出的代码活得久、哪些会被返工。独立开发者没有同事、没有代码审查、常常也没有用户，现实的反馈到不了他们那里。

Prudence 就是这份反馈。它读取编码代理本来就存在磁盘上的会话记录（目前是 Claude Code，其他代理已在设计和计划中），把这些会话产出的每一行代码沿着 git 追踪下去，然后告诉你发生了什么：哪些活了下来、哪些被重写、token 和时间花在了什么上、你自己的哪些习惯和哪些结果同时出现。记录属于你，留在你的机器上。
...
```
Claude Code 记录和钩子 ──► 一个本地 SQLite 库（原始归档 + 派生表）
        ──► 归因（哪个会话产出了哪个提交，带置信度）
        ──► 结果（存活、返工）、事实、观察、回顾
...
0.3.0。目前读取的代理是 Claude Code；数据源层按代理一个模块设计，Codex 适配器是下一个。Windows 未测试。见 [RELEASES.md](RELEASES.md)。
