...
- **看清时间和 token 花在了哪里，换来的代码后来怎么样了。** 按模型每次回复做了什么统计 token；每一行代码沿着 git 追踪：7、30、90 天后是否还活着，是否被你自己后来的提交返工，每个数字都带覆盖率。
...
- **按做了什么统计的用量。** 模型的每一次回复只看它调用了哪些工具，归入四类之一：改了文件、运行了什么、只读、只是对话。token 按项目和按周汇总，产出零代码的会话会显示它的 token 花在了什么上，而不是看起来像一个空洞。不读任何文本，也没有阈值，同一份历史永远得到同一个划分。
...
prudence usage --last 30d                 # 按改动、运行、阅读、对话统计的 token
