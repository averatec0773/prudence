"""How many times this session dispatched a subagent, through the `Task` or `Agent` tool
(Claude Code renamed the tool; both names occur in real transcripts).
...
def compute(connection: sqlite3.Connection, session_id: str) -> int:
    (count,) = connection.execute(
        "SELECT COUNT(*) FROM tool_call WHERE session_id = ? AND tool_name IN ('Task', 'Agent')",
        (session_id,),
    ).fetchone()
    return count
